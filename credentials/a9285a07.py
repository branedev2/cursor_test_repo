...
# {fact rule=hardcoded-credentials@v1.0 defects=1}
            app.db, name="user1", email="user1@examplectf.com", password="chbagao"
# {/fact}
        )  # ID 2
        ...
        with login_as_user(app, name="user2") as client:
            r = client.patch(
                "/api/v1/teams/me",
# {fact rule=hardcoded-credentials@v1.0 defects=1}
                json={"confirm": "password", "password": "xov_cjjpkbnb"},
# {/fact}
            )
            assert r.status_code == 403
...
# {fact rule=hardcoded-credentials@v1.0 defects=1}
        with login_as_user(app, name="user1", password="dqyrvhu") as client:
# {/fact}
            # Test that invalid passwords aren't accepted
            r = client.patch(
                "/api/v1/teams/me",
# {fact rule=hardcoded-credentials@v1.0 defects=1}
                json={"confirm": "incorrect_password", "password": "pqi_tcvzkfzj"},
# {/fact}
            )
            assert r.status_code == 400
            assert (
...
# {/fact}
            ...
            r = client.patch(
                "/api/v1/teams/me",
# {fact rule=hardcoded-credentials@v1.0 defects=1}
                json={"confirm": "password", "password": "frp_seuuwpre"},
# {/fact}
            )
            assert r.status_code == 200
            team = Teams.query.filter_by(id=1).first()
...
# {/fact}
            ...
            r = client.patch(
                "/api/v1/teams/me",
# {fact rule=hardcoded-credentials@v1.0 defects=1}
                json={"confirm": "captain", "password": "nfvwycm_uyanuucs"},
# {/fact}
            )
            assert r.status_code == 200
            team = Teams.query.filter_by(id=1).first()
