...
public class Foo1
{
	...
	{   //{fact rule=sensitive-information-leak@v1.0 defects=1}
		SslCertificateTrust val = SslCertificateTrust.CreateForX509Collection(certCollection, true);
	}
	//{/fact}
	private void SomeFunction2(string arg1, X509Certificate2Collection certCollection)
	{   //{fact rule=sensitive-information-leak@v1.0 defects=1}
		SslCertificateTrust val = SslCertificateTrust.CreateForX509Collection(certCollection, sendTrustInHandshake = true);
	}
	//{/fact}
	...
	{
		//{fact rule=sensitive-information-leak@v1.0 defects=1}
		SslCertificateTrust val = SslCertificateTrust.CreateForX509Store(certCollection, true);
	}
	//{/fact}
	private void SomeFunction5(string arg1, X509Store certCollection)
	{	
		//{fact rule=sensitive-information-leak@v1.0 defects=1}
		SslCertificateTrust val = SslCertificateTrust.CreateForX509Store(certCollection, sendTrustInHandshake = true);
	}
	//{/fact}
